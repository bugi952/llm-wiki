...
[Filter A] 주제 분류 — "AI야? Macro야? 무관?"
    → Claude CLI (Haiku). 무관한 거 80% 탈락
    ↓
[Filter B] 품질+신규성 — "이미 아는 내용? 새 인사이트?"
    → Claude CLI가 Wiki 기존 지식 대조. 1~5점. 3점+ 통과
    ↓
...
런타임:      Python
LLM:        Claude Code CLI (Haiku, Max 구독 — API 비용 $0)
...
인프라:      $0 (기존 VPS 공유)
API:        $0 (전부 무료 tier)
LLM 필터링:  $0 (Claude Code CLI — Max 구독에 포함, 하루 ~70회 Haiku)
동기화:      $0 (GitHub 무료)
---
총:         $0 (Claude Max 월구독 외 추가 비용 없음)
```
...
| Claude API (anthropic SDK) | Max 구독으로 CLI 사용 가능. 추가 API 비용 불필요 |
