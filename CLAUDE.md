...
- LLM: OpenAI Codex CLI + ChatGPT Plus 구독 (VPS에서 `codex login --device-auth` 완료)
...
## Tech Stack
Python 3.12+ / OpenAI Codex CLI (ChatGPT Plus 구독) / SQLite / feedparser / Telegram Bot API / GitPython / crontab + systemd

## Hard Rules

### Tier 0 -- 절대 불변 (어떤 상황에서도 위반 금지)
- **설계 문서가 진리.** docs/의 설계가 모든 구현 결정의 근거. 임의 변경/확장 금지
- **CLI 호출 최소화.** domain 고정 소스는 Filter A CLI 스킵. indexer는 증분만. 모든 LLM 호출은 `codex exec` subprocess
- **vault/raw/는 Git에 안 올림.** .gitignore 필수. PDF는 VPS에만 보관
- **수동 인풋은 즉시 처리.** 건재가 텔레그램으로 던지면 크론 대기 없이 즉시 파이프라인
- **추가 비용 $0.** 무료 API만 사용. Codex는 ChatGPT Plus 구독 CLI로 $0
