...
## Tech Stack
Python 3.11+ / Claude Code CLI (Haiku, Max 구독) / SQLite / feedparser / Telegram Bot API / GitPython / crontab + systemd
...
### 착수 후 -- Superpowers 워크플로우
- writing-plans로 현재 Phase 수락 조건을 task로 분해
- subagent-driven-development로 task 실행 (task마다 fresh subagent + 2단계 리뷰)
- test-driven-development: RED/GREEN/REFACTOR 필수
...
이 프로젝트는 Superpowers 플러그인이 설치되어 있다.
- brainstorming: **건너뛰기**. docs/가 이미 존재한다
- writing-plans: Phase별로 실행. build-phase 수락 조건 기반 task 분해
- subagent-driven-development: 기본 실행 방식
- test-driven-development: 모든 구현에 적용
- 우선순위: CLAUDE.md Hard Rules > Superpowers skills > 기본 행동
